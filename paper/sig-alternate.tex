...
% Gustavo: tentei mostrar estudos relacionados aqiu, mas acho que poderia ter vendido melhor
...
%Aniche, será que daria pra tratar de forma diferenciada (e.g., contando como 2 unit tests) o seguinte caso:
%você invocar o mesmo método método de produção duas vezes dentro de um unit test, mas você o faz com input parameters diferentes.
%Nesse caso, pode ser que a intenção tenha sido de exercitar duas branches do mesmo método de produção (mesmo sendo bad practice fazer isso
%dentro de um único test case)
...
%Faltou algo importante: mostrar o quão melhor esses valores são em comparação 
%aos anteriores (usando tua implementação)
